...
function setParams(_params) {
  ...
  switch (_params.type) {
    ...
    case "multisig":
      ...
      break;
    default:
      console.error("Unkown contract type");
  }
  // Construct variables
  params.eventName = params.Abi[0].name;
  params.eventVarName = params.Abi[0].inputs[1].name;
      ...
      web3local.eth.getTransaction(tx.transactionHash).then(txData => {
        return {
          ...
          value: web3local.utils.fromWei(
            tx.returnValues[params.eventVarName],
            "ether"
          ),
          message: txData.input.length
            ? web3local.utils.hexToAscii(txData.input)
            : "",
          link: blockExplorerLink + String(txData.hash)
        };
      })
    )
  );
...
function subscribeToDonations(callback) {
  params.contract.events[params.eventName]({
    fromBlock: "latest"
  }).on("data", callback);
}
